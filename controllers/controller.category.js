                    ...
                    pipeline: [
                        {
                            $match: {
                                $expr: {
                                    $in: ["$$category_name", { $map: { input: "$cat", as: "c", in: { $toLower: "$$c" } } }]
                                }
                            }
                        }
                    ],
...
let addNewCategory = async (req, res) => {
    try {
        ...
        if (validator(args)) {
            ...
            await cat.save();
            res.status(200).json({ status: "success", message: "Category is added successfully", data: cat });
        } else {
...
let updateCategory = async (req, res) => {
    try {
        ...
        if (!old) {
            return res.status(404).json({ status: "fail", msg: `No category found with this id: ${id}` });
        }
        ...
        if (isSuccess) {
            return res.json({ status: "success", msg: "Category and related products updated successfully", data: updatedCategory });
        } else {
            return res.status(500).json({ status: "fail", msg: "Failed to update related products" });
        }
    } catch (error) {
